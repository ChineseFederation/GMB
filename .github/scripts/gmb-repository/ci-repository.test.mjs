...
// ../../../private/var/folders/z1/h1pvtv0x76xg5h60y_2npmbc0000gn/T/gmb-script-tests-BK47jS/mobile-store.test.mjs
...
// ../../../private/var/folders/z1/h1pvtv0x76xg5h60y_2npmbc0000gn/T/gmb-script-tests-BK47jS/mobile-store.mjs
import { createHash, createPrivateKey, sign } from "node:crypto";
import { readFileSync as readFileSync2, writeFileSync } from "node:fs";
import { basename as basename2, resolve as resolve2 } from "node:path";
import { fileURLToPath as fileURLToPath2 } from "node:url";
var APPLE_API = "https://api.appstoreconnect.apple.com/v1";
var GOOGLE_API = "https://androidpublisher.googleapis.com";
var GOOGLE_TOKEN = "https://oauth2.googleapis.com/token";
var acceptedAppleStates = /* @__PURE__ */ new Set([
  "ACCEPTED",
  "IN_REVIEW",
...
var products = Object.freeze({
  citizenapp: Object.freeze({
    title: "\u516C\u6C11",
    bundleId: "ios.citizenapp",
    packageName: "com.crcfrcn.citizenapp"
  }),
  citizenwallet: Object.freeze({
    title: "\u516C\u6C11\u94B1\u5305",
    bundleId: "ios.citizenwallet",
    packageName: "com.crcfrcn.citizenwallet"
  })
});
function fail2(message) {
  throw new Error(message);
}
function exactKeys2(value2, expected, label) {
  if (!value2 || Array.isArray(value2) || typeof value2 !== "object" || Object.keys(value2).sort().join(",") !== [...expected].sort().join(",")) {
    fail2(`${label}\u5B57\u6BB5\u96C6\u5408\u65E0\u6548`);
  }
}
...
function stableJson(value2) {
  ...
  if (value2 && typeof value2 === "object") {
    return `{${Object.keys(value2).sort().map((key) => `${JSON.stringify(key)}:${stableJson(value2[key])}`).join(",")}}`;
  }
...
function parseArgs2(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!key?.startsWith("--") || value2 === void 0 || Object.hasOwn(values, key.slice(2))) {
      fail2(`\u53C2\u6570\u683C\u5F0F\u65E0\u6548\uFF1A${key || ""}`);
    }
    values[key.slice(2)] = value2;
  }
  for (const key of ["platform", "product", "version-tag", "manifest", "asset", "receipt"]) {
    if (!values[key]) fail2(`\u7F3A\u5C11\u53C2\u6570 --${key}`);
  }
  return values;
}
function verifyMobileStoreInput({ platform, productId, versionTag, manifestPath, assetPath, sourceSha }) {
  const product = products[productId];
  if (!product || !["ios", "android"].includes(platform)) fail2("\u5E94\u7528\u5546\u5E97\u4EA7\u54C1\u6216\u5E73\u53F0\u672A\u767B\u8BB0");
  const manifest = JSON.parse(readFileSync2(resolve2(manifestPath), "utf8"));
  exactKeys2(manifest, [
    "product_id",
    "version",
    "build_number",
  ...
  if (manifest.product_id !== productId || manifest.bundle_id !== product.bundleId || manifest.package_name !== product.packageName || !/^\d+\.\d{1,2}\.\d{1,2}$/.test(manifest.version) || !Number.isSafeInteger(manifest.build_number) || manifest.build_number <= 0 || !/^[0-9a-f]{40}$/.test(manifest.head_sha) || manifest.head_sha !== sourceSha || versionTag !== `${productId}-${platform}-v${manifest.version}` || !Array.isArray(manifest.assets) || manifest.assets.length === 0) {
    fail2("\u79FB\u52A8\u7AEF Release \u8EAB\u4EFD\u4E0E\u53D1\u5E03\u8F93\u5165\u4E0D\u4E00\u81F4");
  }
  const assetName = basename2(assetPath);
  const expectedName = platform === "ios" ? `${productId}.ipa` : `${productId}.aab`;
  if (assetName !== expectedName) fail2("\u5E94\u7528\u5546\u5E97\u6B63\u5F0F\u8D44\u4EA7\u540D\u65E0\u6548");
  const matches = manifest.assets.filter((asset) => asset?.platform === platform && asset?.asset_name === assetName && /^[0-9a-f]{64}$/.test(asset?.asset_sha256));
  if (matches.length !== 1 || matches[0].asset_sha256 !== sha256(assetPath)) {
    fail2("\u5E94\u7528\u5546\u5E97\u6B63\u5F0F\u8D44\u4EA7\u6458\u8981\u4E0E Release \u6E05\u5355\u4E0D\u4E00\u81F4");
  }
...
function appleJWT() {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(issuer) || !/^[A-Z0-9]{10}$/.test(keyId) || !pem.includes("PRIVATE KEY")) fail2("App Store Connect GitHub Secrets \u683C\u5F0F\u65E0\u6548");
...
function googleTokenAssertion(account) {
  ...
  if (!account || Array.isArray(account) || typeof account !== "object" || required4.some((key) => typeof account[key] !== "string" || account[key].length === 0)) {
    fail2("Google Play \u670D\u52A1\u8D26\u53F7\u5B57\u6BB5\u96C6\u5408\u65E0\u6548");
  }
  if (account.type !== "service_account" || !/^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.iam\.gserviceaccount\.com$/.test(account.client_email) || account.token_uri !== GOOGLE_TOKEN || !String(account.private_key).includes("PRIVATE KEY")) {
    fail2("Google Play GitHub Secret \u683C\u5F0F\u65E0\u6548");
  }
  const now = Math.floor(Date.now() / 1e3);
...
async function request(url, { method = "GET", token = "", headers = {}, body, accepted = [200], timeout = 12e4 } = {}) {
  ...
  try {
    ...
    if (bytes.length > 4 * 1024 * 1024) fail2(`\u8FDC\u7AEF\u54CD\u5E94\u8D85\u8FC7\u9650\u5236\uFF1A${new URL(url).host}`);
    if (!accepted.includes(response.status)) {
      fail2(`\u8FDC\u7AEF\u8BF7\u6C42\u5931\u8D25\uFF1A${new URL(url).host} HTTP ${response.status}`);
    }
...
function appleUrl(path, query = {}) {
  ...
  for (const [key, value2] of Object.entries(query)) url.searchParams.set(key, value2);
  return url;
}
function resources(object) {
  if (!Array.isArray(object?.data)) fail2("Apple API \u8D44\u6E90\u5217\u8868\u65E0\u6548");
  return object.data;
}
async function appleBuild(token, appId, version, buildNumber) {
  ...
  const preReleaseIds = new Set((object.included || []).filter((row) => row?.type === "preReleaseVersions" && row?.attributes?.version === version).map((row) => row.id));
  const matches = resources(object).filter((row) => preReleaseIds.has(row?.relationships?.preReleaseVersion?.data?.id));
  if (matches.length > 1) fail2("Apple \u8FD4\u56DE\u591A\u4E2A\u76F8\u540C\u7248\u672C\u4E0E build number \u7684 Build");
...
async function uploadAppleBuild(token, appId, manifest, assetPath) {
  const upload = await jsonRequest(`${APPLE_API}/buildUploads`, {
    ...
    body: jsonBody({ data: {
      ...
      attributes: {
        ...
        cfBundleVersion: String(manifest.build_number),
        platform: "IOS"
      },
  ...
  if (!uploadId) fail2("Apple Build Upload \u54CD\u5E94\u7F3A\u5C11 id");
  const binary = readFileSync2(assetPath);
  ...
  if (!fileId || !Array.isArray(operations) || operations.length === 0) {
    fail2("Apple Build Upload \u672A\u8FD4\u56DE\u4E0A\u4F20\u64CD\u4F5C");
  }
  for (const operation of operations) {
    const url = new URL(String(operation.url || ""));
    const offset = Number(operation.offset);
    const length = Number(operation.length);
    if (url.protocol !== "https:" || !url.hostname.endsWith(".blobstore.apple.com") || !Number.isSafeInteger(offset) || !Number.isSafeInteger(length) || offset < 0 || length <= 0 || offset + length > binary.length || !["PUT", "POST"].includes(operation.method)) fail2("Apple Build Upload \u5206\u7247\u6307\u4EE4\u65E0\u6548");
    const headers = Object.fromEntries((operation.requestHeaders || []).map((row) => [row.name, row.value]));
    await request(url, {
      method: operation.method,
      headers,
      body: binary.subarray(offset, offset + length),
...
async function submitApple({ manifest, product, assetPath }) {
  ...
  if (apps.length !== 1 || apps[0]?.attributes?.name !== product.title) {
    fail2(`App Store Connect \u8F6F\u4EF6\u540D\u79F0\u5FC5\u987B\u662F\u4E2D\u6587\u201C${product.title}\u201D\u4E14\u5E94\u7528\u8BB0\u5F55\u552F\u4E00`);
  }
  ...
  if (!buildId) {
    ...
    while (!buildId && Date.now() < deadline) {
      await new Promise((resolvePromise) => setTimeout(resolvePromise, 1e4));
      buildId = await appleBuild(appleJWT(), appId, manifest.version, manifest.build_number);
    }
  }
  if (!buildId) fail2("Apple \u5DF2\u63A5\u6536 IPA\uFF0C\u4F46 30 \u5206\u949F\u5185\u672A\u5B8C\u6210 Build \u5904\u7406\uFF1B\u4FDD\u7559\u540C\u7248\u672C\u91CD\u8BD5");
  const currentToken = appleJWT();
  const versions = resources(await jsonRequest(appleUrl("appStoreVersions", {
    "filter[app]": appId,
    "filter[platform]": "IOS",
    "filter[versionString]": manifest.version
  }), { token: currentToken }));
  if (versions.length !== 1) fail2("App Store Connect \u7F3A\u5C11\u51C6\u786E\u7248\u672C\u8BB0\u5F55\u6216\u5B58\u5728\u91CD\u590D\u7248\u672C");
  const versionId = versions[0].id;
  const state = versions[0]?.attributes?.appVersionState;
  ...
  await request(`${APPLE_API}/appStoreVersions/${versionId}/relationships/build`, {
    ...
    token: currentToken,
    headers: { "content-type": "application/json" },
    body: jsonBody({ data: { type: "builds", id: buildId } }).body,
    accepted: [204]
  });
  ...
  if (!submissionId) fail2("Apple Review Submission \u54CD\u5E94\u7F3A\u5C11 id");
...
async function googleAccessToken() {
  let account;
  try {
    account = JSON.parse(String(process.env.PLAY_KEY || ""));
  } catch {
    fail2("PLAY_KEY \u4E0D\u662F\u6709\u6548 JSON");
  }
  ...
  if (!response.access_token || String(response.access_token).length < 20) fail2("Google OAuth \u672A\u8FD4\u56DE\u6709\u6548 access token");
...
async function submitGoogle({ manifest, product, assetPath }) {
  ...
  if (!/^[A-Za-z0-9._-]+$/.test(String(editId || ""))) fail2("Google Play edit \u54CD\u5E94\u7F3A\u5C11\u6709\u6548 id");
  const exact = `${root}/${editId}`;
  let committed = false;
  try {
    const listings = await jsonRequest(`${exact}/listings`, { token });
    if (!Array.isArray(listings.listings) || listings.listings.length === 0 || !listings.listings.every((row) => row.title === product.title)) {
      fail2(`Google Play \u8F6F\u4EF6\u540D\u79F0\u5FC5\u987B\u5168\u90E8\u662F\u4E2D\u6587\u201C${product.title}\u201D`);
    }
    ...
    const exists = (track.releases || []).some((release2) => (release2.versionCodes || []).includes(versionCode));
    if (!exists) {
      ...
      const uploaded = await jsonRequest(upload, {
        ...
        body: readFileSync2(assetPath),
        timeout: 10 * 6e4
      });
      if (String(uploaded.versionCode) !== versionCode) fail2("Google Play \u8FD4\u56DE\u7684 versionCode \u4E0E Release \u4E0D\u4E00\u81F4");
    }
    await jsonRequest(trackUrl, {
      ...
      body: jsonBody({ track: "production", releases: [{
        name: `${product.title} ${manifest.version}`,
        status: "completed",
...
async function publishMobileStore(input2) {
  const verified = verifyMobileStoreInput(input2);
  const result = input2.platform === "ios" ? await submitApple({ ...verified, assetPath: input2.assetPath }) : await submitGoogle({ ...verified, assetPath: input2.assetPath });
  return {
    product_id: input2.productId,
    platform: input2.platform,
    software_flow: "publish",
    software_version: verified.manifest.version,
    build_number: verified.manifest.build_number,
    source_sha: verified.manifest.head_sha,
    version_tag: input2.versionTag,
    asset_sha256: verified.assetSha256,
...
if (false && isMain) {
  try {
    ...
    const receipt2 = await publishMobileStore({
      platform: args.platform,
      productId: args.product,
      versionTag: args["version-tag"],
      manifestPath: args.manifest,
      assetPath: args.asset,
      sourceSha: String(process.env.GMB_SOURCE_SHA || "")
    });
    writeFileSync(resolve2(args.receipt), `${stableJson(receipt2)}
`, { mode: 384, flag: "wx" });
    process.stdout.write(`\u5E94\u7528\u5546\u5E97\u5DF2\u63A5\u53D7 ${receipt2.product_id} ${receipt2.platform} ${receipt2.software_version}
`);
  } catch (error) {
...
// ../../../private/var/folders/z1/h1pvtv0x76xg5h60y_2npmbc0000gn/T/gmb-script-tests-BK47jS/mobile-store.test.mjs
function fixture2(platform = "ios") {
  const root = mkdtempSync(join2(tmpdir(), "gmb-mobile-store."));
  const productId = "citizenapp";
  const assetName = `${productId}.${platform === "ios" ? "ipa" : "aab"}`;
  const assetPath = join2(root, assetName);
  writeFileSync2(assetPath, "fixture-release-asset");
  const digest = createHash2("sha256").update("fixture-release-asset").digest("hex");
  const manifestPath = join2(root, `${productId}-release-${platform}.json`);
...
test3("\u79FB\u52A8\u7AEF\u53D1\u5E03\u53EA\u63A5\u53D7\u51C6\u786E Release Tag\u3001\u6E90\u7801\u3001\u4EA7\u54C1\u7AEF\u548C\u8D44\u4EA7\u6458\u8981", () => {
  const value2 = fixture2();
  try {
    const result = verifyMobileStoreInput(value2);
    assert3.equal(result.assetName, "citizenapp.ipa");
    assert3.equal(result.manifest.build_number, 7);
  } finally {
    rmSync(value2.root, { recursive: true, force: true });
  }
});
test3("\u79FB\u52A8\u7AEF\u53D1\u5E03\u62D2\u7EDD\u6458\u8981\u6F02\u79FB\u548C\u8DE8\u7AEF Tag", () => {
  const value2 = fixture2("android");
  try {
    writeFileSync2(value2.assetPath, "tampered");
    assert3.throws(() => verifyMobileStoreInput(value2), /摘要/u);
    writeFileSync2(value2.assetPath, "fixture-release-asset");
    assert3.throws(() => verifyMobileStoreInput({ ...value2, versionTag: "citizenapp-ios-v1.2.3" }), /身份/u);
  } finally {
    rmSync(value2.root, { recursive: true, force: true });
  }
});

// ../../../private/var/folders/z1/h1pvtv0x76xg5h60y_2npmbc0000gn/T/gmb-script-tests-BK47jS/publish-record.test.mjs
import assert4 from "node:assert/strict";
import { readFileSync as readFileSync4 } from "node:fs";
import test4 from "node:test";

// ../../../private/var/folders/z1/h1pvtv0x76xg5h60y_2npmbc0000gn/T/gmb-script-tests-BK47jS/publish-record.mjs
...
var FAILED_DEPLOYMENT_STATES = /* @__PURE__ */ new Set(["error", "failure"]);
function exactKeys3(value2, expected) {
  if (Object.keys(value2).sort().join(",") !== [...expected].sort().join(",")) {
    fail3("GitHub \u53D1\u5E03\u56DE\u6267\u5B57\u6BB5\u96C6\u5408\u65E0\u6548");
  }
}
function keepLatestFailure(deployments, createdAt = Date.parse) {
  const failed = deployments.filter((value2) => FAILED_DEPLOYMENT_STATES.has(String(value2?.state || "")) && value2?.id !== void 0).map((value2) => ({
    ...value2,
    createdAt: Number.isNaN(createdAt(value2.created_at || "")) ? 0 : createdAt(value2.created_at || "")
...
function parseArgs3(argv) {
  ...
  for (const key of ["receipt", "environment", "description", "url"]) {
    if (!values[key]) fail3(`\u7F3A\u5C11\u53C2\u6570 --${key}`);
  }
  return values;
}
function verifyPublishReceipt(receipt2, { environment, url }) {
  const targets = {
    ...
    "citizenserve-cloudflare-production": ["citizenserve", "cloudflare", "https://www.crcfrcn.com/api/health"],
    "citizenweb-production": ["citizenweb", null, "https://www.crcfrcn.com/"]
  };
  const target = targets[environment];
  if (!receipt2 || Array.isArray(receipt2) || typeof receipt2 !== "object" || !target || url !== target[2] || receipt2.product_id !== target[0] || receipt2.software_flow !== "publish" || !/^\d+\.\d{1,2}\.\d{1,2}$/.test(String(receipt2.software_version || "")) || !/^[0-9a-f]{40}$/.test(String(receipt2.source_sha || "")) || !["reviewing", "published"].includes(receipt2.publish_state) || !/^[A-Za-z0-9._-]{1,256}$/.test(String(receipt2.publish_receipt_id || ""))) {
    fail3("GitHub \u53D1\u5E03\u56DE\u6267\u8EAB\u4EFD\u65E0\u6548");
  }
  const base = [
    "product_id",
    "software_flow",
  ...
  if (target[1] === null) {
    exactKeys3(receipt2, base);
    if (receipt2.version_tag !== `${receipt2.product_id}-v${receipt2.software_version}` || receipt2.publish_state !== "published") fail3("GitHub \u53D1\u5E03\u56DE\u6267\u8EAB\u4EFD\u65E0\u6548");
  } else if (["ios", "android"].includes(target[1])) {
    exactKeys3(receipt2, [...base, "platform", "build_number", "asset_sha256"]);
    if (receipt2.platform !== target[1] || receipt2.version_tag !== `${receipt2.product_id}-${receipt2.platform}-v${receipt2.software_version}` || !Number.isSafeInteger(receipt2.build_number) || receipt2.build_number <= 0 || !/^[0-9a-f]{64}$/.test(String(receipt2.asset_sha256 || ""))) {
      fail3("GitHub \u53D1\u5E03\u56DE\u6267\u8EAB\u4EFD\u65E0\u6548");
    }
  } else {
    exactKeys3(receipt2, [...base, "target"]);
    if (receipt2.target !== target[1] || receipt2.version_tag !== `${receipt2.product_id}-${receipt2.target}-v${receipt2.software_version}` || receipt2.publish_state !== "published") fail3("GitHub \u53D1\u5E03\u56DE\u6267\u8EAB\u4EFD\u65E0\u6548");
  }
  return receipt2;
}
function githubClient({ repository, token }) {
  if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository) || token.length < 20) {
    fail3("GitHub \u53D1\u5E03\u8BB0\u5F55\u73AF\u5883\u65E0\u6548");
  }
  const request2 = async (path, body) => {
    ...
    if (!response.ok) fail3(`GitHub \u53D1\u5E03\u8BB0\u5F55\u5931\u8D25\uFF1AHTTP ${response.status}`);
    return text ? JSON.parse(text) : {};
  };
  const parseDate = (value2) => Number(Date.parse(String(value2 || "")));
  return {
    request: request2,
    async listDeployments(environment) {
      const values = [];
      for (let page = 1; page <= 20; page += 1) {
        const output = await request2(`deployments?environment=${encodeURIComponent(environment)}&per_page=100&page=${page}`);
        required(Array.isArray(output), "GitHub \u53D1\u5E03\u8BB0\u5F55\u5217\u8868\u65E0\u6548");
        values.push(...output);
        if (output.length < 100) return values;
      }
      fail3("GitHub \u53D1\u5E03\u8BB0\u5F55\u5217\u8868\u8D85\u8FC7\u5B89\u5168\u5206\u9875\u4E0A\u9650");
    },
    async deleteDeployment(deploymentId) {
      if (!Number.isSafeInteger(deploymentId) || deploymentId <= 0) fail3("GitHub \u90E8\u7F72 id \u65E0\u6548");
      await request2(`deployments/${deploymentId}`, { method: "DELETE" });
    }
  };
}
async function pruneOldFailedDeployments(client2, environment) {
  const deployments = await client2.listDeployments(environment);
  const failed = keepLatestFailure(deployments);
  if (failed.length <= 1) return;
  for (const deployment of failed.slice(1)) {
    await client2.deleteDeployment(deployment.id);
  }
}
async function recordPublish({ receipt: receipt2, environment, description, url, client: client2 }) {
  verifyPublishReceipt(receipt2, { environment, url });
  await pruneOldFailedDeployments(client2, environment);
  const deployment = await client2.request("deployments", {
    ref: receipt2.version_tag,
    environment,
    description,
    auto_merge: false,
    required_contexts: [],
  ...
  if (!Number.isSafeInteger(deployment.id) || deployment.id <= 0) fail3("GitHub Deployment id \u65E0\u6548");
  await client2.request(`deployments/${deployment.id}/statuses`, {
    state: "success",
    environment,
    environment_url: url,
    ...
    description: `\u53D1\u5E03\u5DF2\u63A5\u53D7\uFF1A${receipt2.publish_state}`
  });
  return deployment.id;
}
var isMain2 = process.argv[1] && resolve3(process.argv[1]) === resolve3(fileURLToPath3(import.meta.url));
if (false && isMain2) {
  try {
    const args = parseArgs3(process.argv.slice(2));
    const receipt2 = JSON.parse(readFileSync3(resolve3(args.receipt), "utf8"));
...
var receipt = Object.freeze({
  ...
  version_tag: "citizenapp-ios-v1.2.3",
  build_number: 7,
  asset_sha256: "b".repeat(64),
  source_sha: "a".repeat(40),
  publish_state: "reviewing",
  publish_receipt_id: "fixture"
});
test4("\u53D1\u5E03\u56DE\u6267\u53EA\u63A5\u53D7\u51C6\u786E GitHub \u6B63\u5F0F\u7248\u672C\u8EAB\u4EFD", () => {
  assert4.equal(verifyPublishReceipt(receipt, {
    environment: "citizenapp-ios-production",
    url: "https://apps.apple.com/"
  }), receipt);
  assert4.throws(() => verifyPublishReceipt({ ...receipt, software_flow: "release" }, {
    environment: "citizenapp-ios-production",
    url: "https://apps.apple.com/"
  }), /身份/u);
  assert4.throws(() => verifyPublishReceipt({ ...receipt, publish_receipt_id: "" }, {
    environment: "citizenapp-ios-production",
    url: "https://apps.apple.com/"
  }), /身份/u);
  assert4.throws(() => verifyPublishReceipt({ ...receipt, extra: true }, {
    environment: "citizenapp-ios-production",
    url: "https://apps.apple.com/"
  }), /字段集合/u);
  const web = {
    ...
    software_flow: "publish",
    software_version: "1.2.3",
    version_tag: "citizenweb-v1.2.3",
    source_sha: "a".repeat(40),
    publish_state: "published",
    publish_receipt_id: "42"
  };
  assert4.equal(verifyPublishReceipt(web, {
    environment: "citizenweb-production",
  ...
  const citizenServe = {
    ...
    software_flow: "publish",
    software_version: "1.2.3",
    version_tag: "citizenserve-cloudflare-v1.2.3",
    source_sha: "a".repeat(40),
    publish_state: "published",
    publish_receipt_id: "worker-version-id"
  };
  assert4.equal(verifyPublishReceipt(citizenServe, {
    environment: "citizenserve-cloudflare-production",
    url: "https://www.crcfrcn.com/api/health"
  }), citizenServe);
  assert4.throws(() => verifyPublishReceipt({ ...citizenServe, target: "web" }, {
    environment: "citizenserve-cloudflare-production",
    url: "https://www.crcfrcn.com/api/health"
  }), /身份/u);
});
test4("GitHub Deployment \u5148\u521B\u5EFA\u8BB0\u5F55\u518D\u5199\u6210\u529F\u72B6\u6001", async () => {
  const calls = [];
  const id = await recordPublish({
    receipt,
    environment: "citizenapp-ios-production",
    description: "\u516C\u6C11 iOS \u53D1\u5E03",
    url: "https://apps.apple.com/",
    client: {
      ...
      async deleteDeployment() {
        throw new Error("\u6CA1\u6709\u65E7\u5931\u8D25\u8BB0\u5F55\u65F6\u4E0D\u5F97\u5220\u9664 Deployment");
      },
  ...
  assert4.equal(id, 42);
  assert4.deepEqual(calls.map((call) => call.path), ["deployments", "deployments/42/statuses"]);
  assert4.equal(calls[0].body.ref, receipt.version_tag);
  assert4.equal(calls[1].body.state, "success");
});
test4("\u516D\u6761\u53D1\u5E03 Workflow \u53EA\u6D88\u8D39 GitHub Release \u4E14\u4E0D\u7F16\u8BD1\u4EA7\u54C1", () => {
  const workflows = [
    "citizenapp/publish-ios.yml",
    "citizenapp/publish-android.yml",
    "citizenwallet/publish-ios.yml",
    "citizenwallet/publish-android.yml",
    "citizenserve/publish-cloudflare.yml",
    "citizenweb/publish-web.yml"
  ];
  for (const workflow of workflows) {
    const source = readFileSync4(new URL(`../../workflows/${workflow}`, import.meta.url), "utf8");
    assert4.match(source, /workflow_dispatch:/);
    assert4.match(source, /source_sha:/);
    assert4.match(source, /version_tag:/);
    assert4.match(source, /gh release download/);
    assert4.match(source, /publish-[^\s]+\.mjs publish-record/);
    assert4.match(source, /runs-on: ubuntu-24\.04/);
    assert4.doesNotMatch(source, /flutter build|xcodebuild|cargo build|upload-artifact/);
    assert4.doesNotMatch(source, /\/Users\/rhett|CitizenConsoleMobileStore/);
  }
});
test4("\u516C\u6C11\u7F51\u53D1\u5E03\u4FDD\u7559\u9884\u89C8\u9010\u6587\u4EF6\u9A8C\u6536\u548C\u751F\u4EA7\u5931\u8D25\u56DE\u6EDA", () => {
  const source = readFileSync4(new URL("../../workflows/citizenweb/publish-web.yml", import.meta.url), "utf8");
  assert4.match(source, /--branch citizenweb-candidate/);
  assert4.match(source, /manifest\.files\.filter/);
  assert4.match(source, /createHash.*require\('node:crypto'\)/);
  assert4.match(source, /待发布官网版本.*必须高于当前生产版本/);
  assert4.match(source, /\/deployments\/\$\{encodeURIComponent\(id\)\}\/rollback/);
  assert4.match(source, /回滚后的正式域名版本标记不是旧版本/);
  assert4.match(source, /verify_deployment 'https:\/\/www\.crcfrcn\.com'/);
});
test4("\u516C\u6C11\u540E\u7AEF\u53D1\u5E03\u56DE\u6EDA\u540E\u590D\u6838\u65E7 Worker \u7248\u672C", () => {
  const source = readFileSync4(new URL(
    "../../workflows/citizenserve/publish-cloudflare.yml",
    import.meta.url
  ), "utf8");
  assert4.match(source, /versions deploy "\$\{stable\}@100"/);
  assert4.match(source, /&& health "\$stable"/);
  assert4.match(source, /已恢复并验证旧 Worker 版本/);
});
