...
`/Users/rhett/Only/console/target/citizensdk/.work`，临时 example 直接调用生产
...
复现时必须继续在上述 Console 隔离目录中使用 CitizenChain 固定 `Cargo.lock` 和
`rust-toolchain.toml`，把 `CARGO_TARGET_DIR` 指向
`/Users/rhett/Only/console/target/citizensdk/.work/runtime-fixture/cargo-target`，连续生成两次并逐字节比较 metadata、events 后
