...
///
/// CitizenSDK 只允许通过本类执行 `mnemonic + password → MiniSecretKey`。助记词按
...
final class WalletMiniSecret {
  ...
  static Future<Uint8List> fromMnemonic(
    ...
    final entropy = Uint8List.fromList(
      Mnemonic.fromSentence(mnemonic, Language.english).entropy,
    );
    List<int>? derived;
    try {
  ...
  ///
  /// 必须复用 `substrate_bip39` 的 [DeriveJunction]，禁止各接入产品重写 SCALE
