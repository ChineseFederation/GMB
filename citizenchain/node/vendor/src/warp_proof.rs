...
impl<Block: BlockT> WarpSyncProof<Block> {
    ...
    {
        // 按请求起点即时生成证明，避免缓存把不同 finalized 起点的响应混用。
        ...
        let set_changes = set_changes
            .iter_from(begin_number)
    ...
    NumberFor<Block>: BlockNumberOps,
{
    fn verify(
        ...
        let proof = WarpSyncProof::<Block>::decode_all(&mut proof.as_slice())
            .map_err(|error| format!("Proof decoding error: {:?}", error))?;
        let last_header = proof
            .proofs
            .last()
            .map(|proof| proof.header.clone())
        ...
        let justifications = proof
            ...
            .map(|proof| {
                let justifications =
                    Justifications::new(vec![(GRANDPA_ENGINE_ID, proof.justification.encode())]);
                (proof.header, justifications)
            })
            .collect::<Vec<_>>();
