...
mod cost {
    ...
    pub(super) const DUPLICATE_NEIGHBOR_MESSAGE: Rep = Rep::new(
        -500,
        "Grandpa: Duplicate neighbor message without grace period",
    );
...
impl<B: BlockT, N: Network<B>, S: Syncing<B>> NetworkBridge<B, N, S> {
...

        {
            ...
            for round in completed.iter() {
                ...
                for signed in round.votes.iter() {
                    ...
                    gossip_engine
                        .lock()
                        .register_gossip_message(topic, message.encode());
                }
    ...
    pub(crate) fn round_communication(
        ...
        impl Stream<Item = SignedMessage<B::Header>> + Unpin,
        OutgoingMessages<B>,
    ) {
...
impl<B: BlockT, N: Network<B>, S: Syncing<B>> Future for NetworkBridge<B, N, S> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
        ...
        loop {
            match self
                .gossip_validator_report_stream
                .lock()
                .poll_next_unpin(cx)
            {
    ...
    gossip_engine
        ...
        .filter_map(move |(notification, msg)| {
            future::ready(match msg {
                GossipMessage::Commit(msg) => process_commit(
                    msg,
                    notification,
                    &gossip_engine,
                    &gossip_validator,
                ...
                GossipMessage::CatchUp(msg) => process_catch_up(
                    msg,
                    notification,
                    &gossip_engine,
                    &gossip_validator,
...
fn check_catch_up<Block: BlockT>(
    ...
    let signatures_checked = check_signatures::<Block, _>(
        msg.prevotes.iter().map(|vote| {
            (
                finality_grandpa::Message::Prevote(vote.prevote.clone()),
                &vote.id,
                &vote.signature,
            )
        }),
