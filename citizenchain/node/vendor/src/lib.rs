...
/// 在数据库裁剪时保留含 GRANDPA justification 的区块，保证 warp sync 证明可生成。
#[derive(Debug, Clone)]
...
impl sc_client_db::PruningFilter for GrandpaPruningFilter {
    fn should_retain(&self, justifications: &sp_runtime::Justifications) -> bool {
        justifications
            .get(sp_consensus_grandpa::GRANDPA_ENGINE_ID)
            .is_some()
    }
}
    ...
    let authority_set_hard_forks = authority_set_hard_forks
        ...
        .map(|fork| {
            let delay_kind = if let Some(last_finalized) = fork.last_finalized {
                authorities::DelayKind::Best {
                    median_last_finalized: last_finalized,
                }
            } else {
...
pub fn grandpa_peers_set_config<B: BlockT, N: NetworkBackend<B, <B as BlockT>::Hash>>(
    ...
    N::notification_config(
        protocol_name,
        grandpa_protocol_name::LEGACY_NAMES
            .iter()
            .map(|&n| n.into())
            .collect(),
        ...
        // PSM Reject → 整个 gossip 失败 → 钱包频繁连不上区块链。
