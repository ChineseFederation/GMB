...
import '../transport/chat_server_transport.dart';
...
/// 宿主只注入身份、用途钥、权益、平台推送和短期 ChatServer 凭证。
abstract interface class ChatRuntimeHost {
  ...
  Future<ChatServerAccess> requestChatServerAccess({
...
class _ChatCrossIsolateCoordinator {
  static const String _pendingMarkerName = '.chat_sdk_data_wipe.pending';
  static const String _pendingMarkerPayload = 'pending\n';
  static const String _completeMarkerName = '.chat_sdk_data_wipe.complete';
  static const String _completeMarkerPayload = 'complete\n';
  static const String _leasePrefix = '.chat_sdk_chat_lease_';
  static const String _leaseSuffix = '.lease';
  static const String _userMutationLeasePrefix = '.chat_sdk_chat_user_';
  static const String _userMutationLeaseSuffix = '.mutation_lease';
  static const String _userMutationStaleSuffix = '.stale';
  static const String _startupBarrierName = '.chat_sdk_chat_startup.barrier';
...
/// 宿主用户 Chat 运行态编排服务。
///
/// 页面层不直接操作 OpenMLS、ChatServer 瞬时转发、近场通道和 Isar。
...
class ChatRuntimeCore {
  ...
  // 或下一条发送再次收敛，避免服务故障时无限请求 ChatServer/D1 会员读。
  ...
  static const String _fileHandoverMacDomain =
      'chat_sdk.local/chat-file-handover|';
  ...
  Future<String> _savePendingDirectPayload({
    ...
    if (!await _host.canSend(account.userId)) {
      throw StateError('当前会话尚未通过 ChatServer 会员鉴权');
    }
    final bindingToken = await _convergeBindingFence(account);
  ...
  /// 用户点击发送时只生成持久待上传密文，不等待 ChatServer、encrypted object storage 或接收设备。
    ...
    for (final userId in inviteeUserIds) {
      ...
      for (final keyPackage in resolved) {
        if (keyPackage.userId != userId) {
          throw StateError('ChatServer 返回的 KeyPackage user ID 与请求目标不一致');
        }
      }
  ...
  Future<List<MlsKeyPackage>> _resolveKeyPackages(
    ...
    if (resolved.isEmpty ||
      ...
      throw StateError('ChatServer 返回的 KeyPackage 当前不可用');
    }
    return resolved;
  }
      ...
      Future<void> refreshMailboxFromCallback() async {
        await session.runCallback(() async {
          try {
            ...
            // 未 ACK 密文仍在 ChatServer；下次推送、启动或恢复前台继续补拉。
          }
        });
      }
  ...
  Future<_ChatServiceContext> _ensureServiceReady({
    ...
    final transport = _transportFactory(
      identity: identity,
      accessProvider: () =>
          _host.requestChatServerAccess(account: account, identity: identity),
    );
  ...
  Future<void> _ensurePushEndpoint({
    ...
    if (cachedExpiresAt - _pushEndpointRefreshSkewMillis > now &&
      ...
      // 本机缓存只能证明上一次输入相同，不能证明 ChatServer 端点仍存在。
  ...
  /// ChatServer 邮箱的唯一入站 Message 边界；服务端路由身份与密文内身份必须一致。
