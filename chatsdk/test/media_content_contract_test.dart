...
void main() {
  ...
  test('attachment transfer URLs require HTTPS', () {
    expect(
      requireAttachmentHttps(
        Uri.parse('https://objects.example.com/a?signature=ok'),
      ).scheme,
      'https',
    );
    expect(
      () => requireAttachmentHttps(Uri.parse('http://objects.example.com/a')),
      throwsArgumentError,
    );
    expect(
      () => requireAttachmentHttps(Uri.parse('https://user@example.com/a')),
      throwsArgumentError,
    );
  });
}
