            ...
            WakeUpReason::IncomingJsonRpcRequest(request_json) => {
                ...
                match request_parsed {
                    ...
                    methods::MethodCall::author_submitExtrinsic { transaction } => {
                        // PoW 链交易池空时不出块，best == finalized == genesis，
                        // submit_and_watch 会死锁（无法本地验证，因为没有 non-finalized block）。
                        // 直接提交不等验证结果，验证交给全节点交易池完成。
                        let tx_len = transaction.0.len();
                        log!(
                            &me.platform,
                            Info,
                            &me.log_target,
                        ...
                        log!(
                            ...
                            &me.log_target,
                            "submit-to-pool",
                            tx_hash = HashDisplay(&transaction_hash),
                        );
