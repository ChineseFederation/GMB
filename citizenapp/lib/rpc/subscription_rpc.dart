...
class SubscriptionRpc {
  ...
  Future<List<ChainCreatorTier>> fetchCreatorPlans(
    ...
    final block = await _rpc.fetchFinalizedBlock();
    final blockHashHex = '0x${_hex(block.blockHash)}';
    return fetchCreatorPlansAtBlock(creatorCidNumber, blockHashHex);
  }
  ...
  Future<List<ChainCreatorTier>> fetchCreatorPlansAtBlock(
    ...
    final data = await _rpc.fetchStorageAtBlock('0x${_hex(key)}', blockHashHex);
    if (data == null) return const <ChainCreatorTier>[];
    final priceTiers = decodeCreatorPlans(data);
    final encodedNames = await Future.wait([
      for (final tier in priceTiers)
        _rpc.fetchStorageAtBlock(
          '0x${_hex(buildCreatorTierNameStorageKey(creatorCidNumber, tier.tierId))}',
          blockHashHex,
        ),
    ]);
