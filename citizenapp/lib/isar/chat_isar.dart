...
/// 用户刚点击发送、尚未取得网络/MLS 上下文时也复用本表：`envelopeId` 以
/// `pending:` 开头且 `envelopeBytesHex` 为空，正文仍由 [plaintextCipher] 加密；
/// 生成正式 Envelope 后在同一事务中用正式行替换，禁止恢复仅存在内存的消息真值。
@collection
class ChatMessageEntity {
  Id id = Isar.autoIncrement;

  /// 本机聊天数据的永久属主 CID；与 envelope ID 组成复合唯一键。
  @Index(composite: [CompositeIndex('envelopeId')], unique: true, replace: true)
  late String ownerCidNumber;
...
class ChatBindingFenceEntity {
  ...
  /// 本机 Chat 数据永久属主 CID。安全门闩禁止 replace，更新必须保留原行与 generation。
  @Index(unique: true)
  late String ownerCidNumber;
  ...
  String? completedSourceGenesisHash;
  int? completedTargetBindingRevision;
  String? completedTargetAccountId;
  String? completedTargetGenesisHash;
...
class ChatIsar {
  ...
  Future<Isar> _open() async {
    ...
    final existing = Isar.getInstance('citizenapp_chat');
  ...
  Future<void> _closeAndDeleteInternal() async {
    ...
    final tracked = _isar;
    final registered = Isar.getInstance('citizenapp_chat');
